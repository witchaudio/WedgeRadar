...
const cacheStore = getStore("idea-machine-cache");
const voteStore = getStore("idea-machine-votes");
...
export async function applyVote({ ideaId, voterId, direction }) {
  ...
  const record = await readVoteRecord(normalizedIdeaId);
  ...
  await writeVoteRecord(normalizedIdeaId, record);
...
async function withVoteData(payload, voterId, cacheKey) {
  return {
    ...
    ideas: await Promise.all(
      (payload.ideas || []).map(async (idea, index) => {
        const stableIdea = attachIdeaIdentity(idea, cacheKey, index);
        const record = await readVoteRecord(stableIdea.id);
...
async function readCacheEntry(key) {
  return cacheStore.get(key, { type: "json", consistency: "strong" });
}

async function writeCacheEntry(key, payload) {
  await cacheStore.setJSON(key, payload);
}

async function readVoteRecord(ideaId) {
  const record = await voteStore.get(ideaId, { type: "json", consistency: "strong" });
  return normalizeVoteRecord(record);
}

async function writeVoteRecord(ideaId, record) {
  if (!record.up && !record.down && !Object.keys(record.voters).length) {
    await voteStore.delete(ideaId);
    return;
  }

  await voteStore.setJSON(ideaId, record);
}

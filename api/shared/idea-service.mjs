...
const tmpDir = "/tmp/idea-machine";
const cacheFile = path.join(tmpDir, "cache.json");
const votesFile = path.join(tmpDir, "votes.json");
...
const KEYWORD_THEME_MAP = {
  ai: "Automation", analytics: "Analytics", app: "Store", apps: "Store",
  authority: "Authority", automation: "Automation", backlink: "Backlink",
  backlinks: "Backlink", b2b: "Sales", churn: "Churn", compliance: "Compliance",
  content: "Content", conversion: "Paywall", creative: "Creative",
  customer: "Support", demand: "Demand", experiment: "Experiment",
  experiments: "Experiment", growth: "Launch", install: "Store", intent: "Intent",
  keyword: "Keyword", keywords: "Keyword", local: "Local", onboarding: "Onboarding",
  paywall: "Paywall", pricing: "Pricing", ranking: "Ranking", retention: "Retention",
  review: "Review", reviews: "Review", sales: "Sales", screenshot: "Screenshot",
  screenshots: "Screenshot", search: "SERP", serp: "SERP", snippet: "Snippet",
  snippets: "Snippet", store: "Store", subscription: "Subscription",
  subscriptions: "Subscription", support: "Support", traffic: "Ranking",
  workflow: "Workflow",
};

export async function getIdeasPayload({ category, intensity, refresh, voterId }) {
  const normalizedCategory = normalizeCategory(category);
  const intensityValue = clamp(Number.parseInt(intensity || "12", 10), 0, 100);
  const intensityBand = getIntensityBand(intensityValue);
  const todayKey = new Date().toISOString().slice(0, 10);
  const cacheKey = `${cacheVersion}:${todayKey}:${normalizedCategory}:${intensityBand.key}`;

  if (!refresh) {
    const cachedPayload = readCacheEntry(cacheKey);
    if (cachedPayload) {
      return withVoteData(cachedPayload, voterId, cacheKey);
    }
  }

  const collected = await collectTrendSignals(normalizedCategory);
  ...
  const payload = {
    category: normalizedCategory,
    categoryLabel: CATEGORY_CONFIG[normalizedCategory].label,
  ...
  return withVoteData(payload, voterId, cacheKey);
}

export async function applyVote({ ideaId, voterId, direction }) {
  const normalizedIdeaId = normalizeIdeaId(ideaId);
  const normalizedVoterId = normalizeVoterId(voterId);
  const normalizedDirection = normalizeVoteDirection(direction);

  if (!normalizedIdeaId || !normalizedVoterId || !normalizedDirection) {
    return { error: "Vote request is missing a valid idea, voter, or direction." };
  }

  const record = readVoteRecord(normalizedIdeaId);
  const previousVote = record.voters[normalizedVoterId] || null;

  if (previousVote === "up") record.up = Math.max(0, record.up - 1);
  if (previousVote === "down") record.down = Math.max(0, record.down - 1);

  let userVote = null;

  if (normalizedDirection === "up") {
    record.up += 1;
    record.voters[normalizedVoterId] = "up";
    userVote = "up";
  } else if (normalizedDirection === "down") {
    record.down += 1;
    record.voters[normalizedVoterId] = "down";
    userVote = "down";
  } else {
    delete record.voters[normalizedVoterId];
  }

  writeVoteRecord(normalizedIdeaId, record);

  return {
    ideaId: normalizedIdeaId,
    votes: { up: record.up, down: record.down },
    userVote,
  };
}
...
function readCacheEntry(key) {
  const store = readJsonFile(cacheFile);
...
function writeCacheEntry(key, payload) {
  const store = readJsonFile(cacheFile);
  store[key] = payload;
  writeJsonFile(cacheFile, store);
}

function readVoteRecord(ideaId) {
  const store = readJsonFile(votesFile);
  return normalizeVoteRecord(store[ideaId]);
}

function writeVoteRecord(ideaId, record) {
  const store = readJsonFile(votesFile);
...
async function collectTrendSignals(category) {
  ...
  return { signals: dedupedSignals.slice(0, 18), unavailableSources };
}

async function generateIdeas({ category, intensityValue, intensityBand, signals }) {
  const groqIdeas = await generateIdeasWithGroq({
    category, intensityValue, intensityBand, signals: signals.slice(0, 12),
  });

  if (groqIdeas?.length) return groqIdeas.slice(0, 9);
...
async function fetchGoogleSignals(query) {
  ...
  return items
    .map((match) => {
      ...
      return buildSignal({ source: "google", title: shortTitle || title, url: link, summary: title, publishedAt: pubDate });
    })
    .filter(Boolean);
}

async function fetchRedditSignals(query) {
  const url = "https://www.reddit.com/search.json?q=" + encodeURIComponent(query) + "&sort=hot&limit=6";
  const data = await fetchJson(url, { headers: { "User-Agent": "IdeaMachine/1.0" } });
...
async function fetchHackerNewsSignals(query) {
  const url = "https://hn.algolia.com/api/v1/search?hitsPerPage=6&tags=story&query=" + encodeURIComponent(query);
...
async function fetchGithubSignals(query) {
  ...
  const headers = { Accept: "application/vnd.github+json", "User-Agent": "IdeaMachine/1.0" };
...
async function fetchXSignals(query) {
  ...
  const url = "https://api.twitter.com/2/tweets/search/recent?max_results=10&tweet.fields=created_at,lang&query=" + encodeURIComponent(`${query} lang:en -is:retweet`);
  const data = await fetchJson(url, { headers: { Authorization: `Bearer ${process.env.TWITTER_BEARER_TOKEN}` } });
...
async function generateIdeasWithGroq({ category, intensityValue, intensityBand, signals }) {
  ...
  const signalLines = signals
    .map((signal, index) => `${index + 1}. [${signal.sourceLabel}] ${signal.title} | ${signal.summary} | ${signal.url}`)
    .join("\n");
  const prompt = [
  ...
  const rawIdeas = await requestGroqIdeas({ prompt, temperature: intensityBand.temperature });
  if (!rawIdeas.length) return null;
  ...
  if (countGenericIdeas(normalizedIdeas) >= 3) {
    const rewritePrompt = [
      ...
      "Trend signals:", signalLines,
      "Draft ideas to improve:", JSON.stringify(normalizedIdeas),
    ].join("\n");
...
async function requestGroqIdeas({ prompt, temperature }) {
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      temperature,
      messages: [
        { role: "system", content: "You are a sharp product strategist who turns live trend signals into concrete software ideas people would actually want to build or buy. You always return strict JSON with no markdown." },
...
function isGenericIdea(idea) {
  ...
  const badTitleWord = /\b(ai|saas|app|tool|platform|marketplace|assistant|dashboard|solution|consultant|specialist|optimizer|tracker|engine)\b/i.test(idea.title || "");
  const genericPatterns = ["ai-powered", "platform for", "tool for", "marketplace for", "assistant to help", "helps businesses", "improve their online presence", "stay competitive", "provides actionable recommendations", "helps a ", "helps businesses"];
...
function generateFallbackIdeas({ category, intensityValue, intensityBand, signals }) {
  ...
  return Array.from({ length: 9 }, (_, index) => {
    ...
    return normalizeIdea({
      title,
      idea: buildFallbackIdeaLine({ audience, themeA, themeB, outcome, category }),
      why: buildFallbackWhy({ primarySignal, secondarySignal }),
      starterPrompt: buildFallbackPrompt({ title, audience, themeA, themeB, category }),
...
function summarizeSignals(signals) {
  ...
  const topSources = [...sourceTotals.entries()].sort((a, b) => b[1] - a[1]).map(([name, count]) => ({ name, count })).slice(0, 5);
  return {
    totalSignals: signals.length,
    topSources,
    headlines: signals.slice(0, 8).map((signal) => ({ source: signal.sourceLabel, title: signal.title, url: signal.url, score: signal.score })),
  };
}
...
function buildSignal({ source, title, url, summary, publishedAt }) {
  ...
  return {
    ...
    source, sourceLabel: meta.label,
    title: truncate(title.trim(), 140), url,
...
function normalizeIdea({ title, idea, why, starterPrompt, sourceMix, trendScore, wildness, signals }) {
  ...
  return {
    ...
    why: truncate(limitSentences(cleanSentence(why) || "Multiple sources are moving around the same customer pain right now.", 2), 190),
    starterPrompt: truncate(cleanSentence(starterPrompt) || "Build a simple MVP for this idea with one main workflow, clear inputs, and a clean results screen.", 220),
...
function inferSources(signals) { return [...new Set(signals.filter(Boolean).map((s) => s.sourceLabel))].slice(0, 3); }
function averageSignalScore(signals) { if (!signals.length) return 68; return Math.round(signals.reduce((sum, s) => sum + s.score, 0) / signals.length); }
function averageWildness(signals) { if (!signals.length) return 24; return clamp(Math.round(35 + signals.length * 4), 10, 80); }
...
function deriveThemes(category, signals) {
  ...
  for (const signal of signals) {
    for (const keyword of signal.keywords || []) {
      ...
      if (mappedTheme && pool.includes(mappedTheme)) matchedThemes.push(mappedTheme);
    }
  }
...
function buildIdeaTitle(themeA, themeB, mechanic) {
  return `${titleCase(themeA)} ${titleCase(themeB)} ${titleCase(mechanic.replace(/\s+/g, " "))}`.replace(/\s+/g, " ").trim();
}

function dedupeSignals(signals) {
  const seen = new Set();
  const uniqueSignals = signals.filter(Boolean).filter((signal) => { const key = signal.title.toLowerCase(); if (seen.has(key)) return false; seen.add(key); return true; }).sort((a, b) => b.score - a.score);
  const buckets = new Map();
  for (const signal of uniqueSignals) { if (!buckets.has(signal.source)) buckets.set(signal.source, []); buckets.get(signal.source).push(signal); }
  const balancedSignals = [];
  let keepLooping = true;
  while (keepLooping) { keepLooping = false; for (const bucket of buckets.values()) { if (!bucket.length) continue; balancedSignals.push(bucket.shift()); keepLooping = true; } }
...
function buildBackupSignals(category) {
  ...
  return examples.map((title, index) => buildSignal({ source: "fallback", title, url: "https://example.com/backup-signal", summary: "Backup signal used when live sources are unavailable.", publishedAt: new Date(Date.now() - index * 60 * 60 * 1000).toISOString() }));
}

function buildFallbackIdeaLine({ audience, themeA, themeB, outcome, category }) {
  const categoryFrame = { all: "idea finder", seo: "SEO tool", aso: "mobile growth tool", saas: "B2B SaaS tool" };
...
function attachIdeaIdentity(idea, cacheKey, index) {
  return { ...idea, id: idea.id || buildIdeaId(cacheKey, index, idea.title), votes: idea.votes || { up: 0, down: 0 }, userVote: idea.userVote || null };
}

function buildIdeaId(cacheKey, index, title) { return `idea_${slugify(`${cacheKey}-${index}-${title}`)}`.slice(0, 160); }

function withVoteData(payload, voterId, cacheKey) {
  return {
    ...payload,
    ideas: (payload.ideas || []).map((idea, index) => {
      const stableIdea = attachIdeaIdentity(idea, cacheKey, index);
      const record = readVoteRecord(stableIdea.id);
      return { ...stableIdea, votes: { up: record.up, down: record.down }, userVote: voterId ? record.voters[voterId] || null : null };
    }),
  };
}
...
function getIntensityBand(value) { return INTENSITY_BANDS.find((band) => value >= band.min && value <= band.max) || INTENSITY_BANDS[0]; }
function normalizeCategory(value) { return value && Object.hasOwn(CATEGORY_CONFIG, value) ? value : "all"; }

function extractXmlValue(block, tagName) { const match = block.match(new RegExp(`<${tagName}>([\\s\\S]*?)<\\/${tagName}>`, "i")); return match ? match[1] : ""; }

function cleanFeedText(value) { return decodeHtmlEntities(value.replace(/<!\[CDATA\[|\]\]>/g, "").replace(/<[^>]+>/g, "").replace(/\s+/g, " ").trim()); }
...
async function fetchJson(url, options = {}) { const response = await fetch(url, options); if (!response.ok) throw new Error(`Request failed: ${response.status}`); return response.json(); }
async function fetchText(url, options = {}) { const response = await fetch(url, options); if (!response.ok) throw new Error(`Request failed: ${response.status}`); return response.text(); }

function safeParseJson(value) {
  if (!value || typeof value !== "string") return null;
  try { return JSON.parse(value); } catch {
    const start = Math.min(...["{", "["].map((t) => value.indexOf(t)).filter((i) => i !== -1));
    const end = Math.max(value.lastIndexOf("}"), value.lastIndexOf("]"));
    if (!Number.isFinite(start) || end === -1) return null;
    try { return JSON.parse(value.slice(start, end + 1)); } catch { return null; }
  }
}

function cleanSentence(value) { if (!value || typeof value !== "string") return ""; return value.replace(/\s+/g, " ").replace(/^["'`]+|["'`]+$/g, "").trim(); }
function limitSentences(value, maxSentences) { const parts = value.match(/[^.!?]+[.!?]?/g) || []; return parts.slice(0, maxSentences).join(" ").trim(); }
...
function freshnessBoost(publishedAt) {
  ...
  if (hoursOld < 24) return 8; if (hoursOld < 72) return 4; if (hoursOld < 168) return 2; return 0;
}

function extractKeywords(text) { return (text.toLowerCase().match(/[a-z0-9]+/g) || []).filter((word) => word.length > 2); }
function shortenHeadline(value) { return truncate(value, 72); }
function truncate(value, maxLength) { if (!value || value.length <= maxLength) return value || ""; return `${value.slice(0, maxLength - 1).trim()}…`; }
function articleFor(value) { return /^[aeiou]/i.test(value.trim()) ? "An" : "A"; }
function normalizeIdeaId(value) { if (typeof value !== "string") return ""; return value.replace(/[^a-z0-9_-]/gi, "").slice(0, 160); }
function normalizeVoterId(value) { if (typeof value !== "string") return ""; return value.toLowerCase().replace(/[^a-z0-9_-]/g, "").slice(0, 64); }
function normalizeVoteDirection(value) { return value === "up" || value === "down" || value === "clear" ? value : ""; }
function titleCase(value) { return value.split(/\s+/).map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(" "); }
function clamp(value, min, max) { if (!Number.isFinite(value)) return min; return Math.min(Math.max(value, min), max); }
function formatDateLabel(date) { return new Intl.DateTimeFormat("en-US", { month: "long", day: "numeric", year: "numeric" }).format(date); }
